...
- Il n'était pas vraiment possible d'étendre les cas présents
...
Pour ce faire, je décide de créer une interface :
...
(Si non founie, je renvoie null par défaut). Je retrouve aussi les génériques utilisés dans mon interface.
...
Rien d'extraordinaire ici, une méthode pour ajouter des règles et une pour définir le default,
puis une méthode pour exécuter le tout : je parcours les règles et je renvois le retour de la première valide.
Je renvoie `this.def` si je n'en trouve pas.
...
ce qui allège partiellement mon code (ouf !) :
...
En faisant ainsi je peux créer autant de règles de la nature souhaitée en spécialisant à volonté.
...
qui retournera ma classe anonyme. Par exemple, si je reprends le code au dessus :
...
Pour conclure cet article, je vais aborder un dernier point sur ce switch qui concerne un problème de
...
et souhaitables. Le dernier exemple de code ajoute d'ailleurs la définition d'une interface pour décrire les expressions
...
D'abord, je peux étendre mon interface avec une classe concrète :
...
Cet article est bien trop grand par rapport à ce que je voulais faire alors je vais conclure avec un dernier exemple
